...
def Numcal(C,L,l0,w1,w2):
    ...
    if L-l0>0:
        ...
        Num[0] =2*np.maximum(np.ceil(4*c1 * epsilon**(-2)/w2),4)/2
...
def DMLEnKBF(C,T,L,lz,l0,z,collection_input):
    Num=Numcal(C,L,l0,w1,w2)
    telescopic_summand=np.zeros((L-l0+1,dim))
    [m,c]=DEnKBF(T,l0,lz,z,int(Num[0]),collection_input)
    telescopic_summand[0]=m[-1,:,0]
    for l in range(l0+1,L+1):
        [m1,m2]=DCEnKBF(T,l,lz,z,int(Num[l-l0]),collection_input)
        telescopic_summand[l-l0]=m1[-1,:,0]-m2[-1,:,0]
    est=np.sum(telescopic_summand,axis=0)
    return est

def PDsimulate_mse(arg_col):
    ...
    #In the following line I'm going to modify the 
    # span of the for to include only the upper level 
        ...
        with progressbar.ProgressBar(max_value=Rep) as bar:
            ...
            for rep in range(Rep):
                with np.errstate(divide='ignore'):
                    ...
                    ML_real[rep] = DMLEnKBF(C,T,l,lz,l0,z,collection_input)
...
if __name__ == '__main__':
    ...
    pool_outputs=pool.map(PDsimulate_mse, inputs)
    ...
    np.savetxt("DMLEnKBF_10blocks.5.txt",MLrl_reshaped ,fmt="%f")
...
DMLEnKBF_10blocks.5.txt blocks from 30 to 49
c1=145863.012072
c2=623.8111694025733
c3= 237040.39353300002
w2=49./50.
...
DMLEnKBF_10blocks.2.txt blocks from 10 to 19
c1=145863.012072
c2=623.8111694025733
c3= 237040.39353300002
w2=49./50.
...
DMLEnKBF_t10blocks_50samples.1.txt blocks from 0 to 9
c1=145863.012072
c2=623.8111694025733
c3= 237040.39353300002
w2=49./50.
...
"DMLEnKBF_10blocks.txt is multilevel for the first 10 blocks
Rep=100
l0=2
L=7
blocks=10
...
"DML500b_lowers.txt" corresponds to the Multilevel algorithm for the efirst 50 blocks
...
"DML500b.txt" corresponds to the Multilevel algorithm for the efirst 40(50?) blocks
with parameters and estimatd constatns 
c1=239308.954479 
c3=1012107.5286470001
c2= 665.4957501794402
